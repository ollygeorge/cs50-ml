...
class MinesweeperAI():
        ...
        for i in range(cell[0]-1, cell[0]+2):
            for j in range(cell[1]-1, cell[1]+2):
                if 0 <= i < self.height and 0 <= j < self.width:
                    ...
                    if test_cell == cell:
                        continue
                    if test_cell in self.mines:
                        mines_counter += 1
                    elif test_cell not in self.safes:
                        unknown_cells.append(test_cell)

        new_sentence = Sentence(unknown_cells, count - mines_counter)
    ...
    def make_safe_move(self):
        ...
        print(possible_safe_moves)
        if possible_safe_moves:
            return random.choice(tuple(possible_safe_moves))
        return None
